import User.Userac as user

if __name__=="__main__":
    #내용이 User.Userac typedls 리스트
    userlist=[]
    #초기 화면, 입력 받을 화면
    print("======Back Menu=====\n", "1. 계좌개설\n", "2. 입금하기\n", "3. 출금하기\n", "4. 전체조회\n", "5. 송금하기\n", "6. 프로그램 종료\n",
          "===================")
    num = int(input("입력 : "))
    #5입력시까지 계속 반복
    while num !=6:

        if(num == 1):
            print("======계좌개설======")
            accnum = int(input("계좌번호 : "))
            name = input("이름 : ")
            depo = int(input("예금 : "))
            #입력받은 계좌, 이름, 예금 정보를 User.Userac 클래스에 입력하고, 입력 정보를 accnum에 저장(여기서 accnum의 type은 char에서 해당 클래스의 type으로 바뀜)
            accnum = user.Userac(name, accnum, depo)
            #userlist에 생성된 유저 정보 저장(이름, 계좌, 예금)
            userlist.append(accnum)
            print("##계좌개설을 완료하였습니다##\n", "==================")

        elif (num == 2):
            print("======입금하기======")
            ind = -1
            accnum = int(input("입금하실 계좌번호를 입력해주세요 : "))
            for i in range(len(userlist)) :
                if userlist[i].user_account() == accnum :
                    ind = i
            if(ind == -1) :
                print("없는 계좌입니다. 처음화면으로 돌아갑니다.")
            else :
                userlist[ind].print_balance()
                userlist[ind].print_name()
                money1 = int(input("입금하실 금액을 입력해주세요 : "))
                userlist[ind].balance = userlist[ind].balance + money1
                print("##계좌잔고 : %d 원##" % userlist[ind].balance)
                print("##입금이 완료되었습니다##\n", "====================")

        elif (num == 3):
            print("======출금하기======")
            ind = -1
            accnum = input("출금하실 계좌번호를 입력해주세요 : ")
            for i in range(len(userlist)) :
                if userlist[i].user_account() == accnum :
                    ind = i
            if (ind == -1):
                print("없는 계좌입니다. 처음 화면으로 돌아갑니다.")
            else:
                userlist[ind].print_balance()
                userlist[ind].print_name()
                money2 = int(input("출금하실 금액을 입력해주세요 : "))
                if (userlist[ind].user_balance() < money2):
                    print("잔액이 부족합니다. 처음 화면으로 돌아갑니다.")
                else :
                    userlist[ind].balance = userlist[ind].balance - money2
                    print("##계좌잔고 : %d 원##" % userlist[ind].balance)
                    print("##출금이 완료되었습니다##\n", "====================")

        elif(num == 4):
            #userlist의 크기(저장된 사람수)로 for문 돌려 0번째부터 userlist의 크기번째까지 모두 출력
            for i in range(len(userlist)):
                userlist[i].print_info()
        elif(num == 5):
            print("======송금하기======")
            indfrom = -1
            indto = -1
            accfrom = int(input("본인 계좌번호를 입력해주세요 : "))
            for i in range(len(userlist)):
                if userlist[i].user_account() == accfrom:
                    indfrom = i
            accto = int(input("송금받을 계좌번호를 입력해주세요 : "))
            for i in range(len(userlist)):
                if userlist[i].user_account() == accto:
                    indto = i
            if(indfrom == -1 or indto == -1):
                print("송금하는 계좌 혹은 송금받는 계좌가 없는 번호입니다. 처음 화면으로 돌아갑니다.")
            else :
                money = int(input("보낼 금액을 입력해주세요 : "))
                if (userlist[indfrom].user_balance() < money):
                    print("잔액이 부족합니다. 처음 화면으로 돌아갑니다.")
                else:
                    userlist[indfrom].balance = userlist[indfrom].balance - money
                    userlist[indto].balance = userlist[indto].balance + money
                    print("##", userlist[indfrom].user_name(), "님의 잔고 : ", userlist[indfrom].user_balance())
                    print("##", userlist[indto].user_name(), "님의 잔고 : ", userlist[indto].user_balance())
                    print("##송금이 완료되었습니다##\n", "====================")
        else:
            print("다시 입력해주세요")

        print("======Back Menu=====\n", "1. 계좌개설\n", "2. 입금하기\n", "3. 출금하기\n", "4. 전체조회\n", "5. 송금하기\n", "6. 프로그램 종료\n",
              "===================")
        num = int(input("입력 : "))
    print("##프로그램을 종료합니다##")